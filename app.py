...
# ── 바탕화면 설치 안내 ─────────────────────────────────────────────────────
# 이 앱 주소만 받아서 처음 들어온 사람 기준으로 쓴다. 저장소를 내려받았거나
# 파이썬이 깔려 있다고 가정하지 않는다 (그쪽 방법은 README에 있다).
with st.expander("💻 바탕화면·홈 화면에 추가하기"):
    st.markdown(
        "지금 보고 계신 이 주소를 앱처럼 만들어 둘 수 있습니다. 설치라고 해도 "
        "따로 내려받는 프로그램은 없고, 브라우저가 바탕화면(또는 홈 화면)에 아이콘을 "
        "하나 만들어 주는 것입니다. 다음부터는 주소를 찾지 않고 아이콘만 누르면 되고, "
        "주소창·탭 없는 창으로 열려서 앱처럼 보입니다."
    )
    ...
    with _t_win:
        st.markdown(
            "**Chrome을 쓰신다면**\n\n"
            "1. 주소창 오른쪽에 있는 **앱에서 열기** 버튼을 누릅니다\n"
            "2. 안내 창이 뜨면 **설치**\n\n"
            "**Edge를 쓰신다면**\n\n"
            "1. 창 오른쪽 위 `···` 를 누릅니다\n"
            "2. **앱** → **이 사이트를 앱으로 설치**\n"
            "3. 이름을 확인하고 **설치**\n\n"
            "끝나면 바탕화면과 시작 메뉴에 아이콘이 생깁니다.\n\n"
            "> 버튼이나 메뉴가 안 보이면 브라우저 버전이 달라 이름이 바뀐 경우입니다. "
            "메뉴에서 **앱**, **바로가기**, **도구 더보기** 안쪽을 살펴보세요."
        )
        if _ICON_ICO.exists():
            st.caption(
                "바탕화면 아이콘이 저울 모양이 아니라 엉뚱하게 보인다면, 아래 파일을 받아 "
                "바로가기 우클릭 → 속성 → 웹 문서 → 아이콘 변경에서 지정하세요."
            )
            st.download_button(
                "⬇️ 아이콘 파일 받기",
    ...
    with _t_mac:
        st.markdown(
            "**Safari** — 위쪽 메뉴 막대의 **파일** → **Dock에 추가**\n\n"
            "**Chrome** — 오른쪽 위 `⋮` → **캐스트, 저장 및 공유** → **페이지를 앱으로 설치**\n\n"
            "**Edge** — 오른쪽 위 `···` → **앱** → **이 사이트를 앱으로 설치**"
        )
    with _t_mobile:
        st.markdown(
            "**아이폰 (Safari)** — 화면 아래 공유 버튼 `⬆️` → 목록을 내려서 **홈 화면에 추가**\n\n"
