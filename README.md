...
4. **"Advanced settings"**(또는 배포 후 앱 설정)의 **Secrets** 칸에 아래 TOML을 붙여넣습니다.
   ...
   `GEMINI_API_KEY` 는 넣지 않습니다. 넣지 않는 것이 기본 동작이며, 접속자가
   각자 사이드바에 자기 키를 입력하게 됩니다.
...
Secrets에 `GEMINI_API_KEY`를 **넣지 않으면 됩니다.** 서버 키는 `ALLOW_SERVER_API_KEY = "1"`
을 명시했을 때만 쓰이므로, 실수로 키가 남아 있어도 남이 내 한도를 쓰지 못합니다.
(로컬에서 혼자 쓸 때는 `.env` 에 이 값을 넣어 두면 키를 매번 입력하지 않아도 됩니다.)
...
### 방법 A — 배포된 앱을 앱처럼 설치 (권장)

인터넷에 배포해 뒀다면 브라우저 기능만으로 끝납니다. 파이썬 설치도 필요 없고
휴대폰에서도 됩니다.

1. Chrome 또는 Edge로 앱 주소 접속
2. 메뉴에서 앱으로 설치
   - **Edge** — 우측 상단 `···` → **앱** → **이 사이트를 앱으로 설치**
   - **Chrome** — 주소창 오른쪽의 **앱에서 열기** 버튼. 없으면 우측 상단 `⋮` →
     **캐스트, 저장 및 공유** → **페이지를 앱으로 설치**
   - 메뉴 이름은 버전마다 다릅니다. 안 보이면 **앱**·**바로가기**·**도구 더보기** 안쪽을 확인하세요.
3. 바탕화면 아이콘이 생기고, 주소창 없는 별도 창으로 열립니다
...
바탕화면에 **일본 판례 검색** 바로가기가 아이콘과 함께 생깁니다. 더블클릭하면
`run_app.bat` 이 실행되어 앱이 뜨고 브라우저가 자동으로 열립니다.

- 처음 한 번은 가상환경 생성과 패키지 설치 때문에 몇 분 걸립니다
  (파이썬이 설치되어 있어야 하며, 설치 시 "Add Python to PATH" 체크 필요)
- 앱을 끄려면 같이 뜬 검은 창을 닫습니다
- 방법 B는 검은 창이 함께 뜨는 것이 정상입니다. 그 창이 곧 서버입니다
